    ...
    def step(self):
        ...
        for layer in self.net.layers:
            for n, dv in layer.grads.items():
                ...
                self.cache[n] = (self.decay * self.cache[n]) + ((1.0 - self.decay) * (np.square(dv)))
    ...
    def step(self):
        ...
        for layer in self.net.layers:
            for n, dv in layer.grads.items():
                ...
                self.mt[n] *= self.beta1
                self.mt[n] += ((1.0 - self.beta1) * dv)
                self.vt[n] *= self.beta2
                self.vt[n] += ((1.0 - self.beta2) * np.square(dv))
                mt_hat = self.mt[n] / (1.0 - (self.beta1 ** self.t))
                vt_hat = self.vt[n] / (1.0 - (self.beta2 ** self.t))
                layer.params[n] -= ((self.lr * mt_hat) / (self.eps + np.sqrt(vt_hat)))
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
